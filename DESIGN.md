...
`agent_done` requires an outcome of `succeeded` or `failed` plus a non-empty summary, and is meant to be called exactly once when the assignment finishes; its model-facing description, prompt snippet, guidelines, and success/error text are generic — they never mention Orca, dispatch ids, the capability token, the CLI command used to report completion, or any private dispatch identifier. Calling `agent_done` returns `terminate: true`, ending the worker's agent run once the report has been sent; it does not itself exit the process. The parsed dispatch metadata (worker handle, capability, dispatch id, task id) stays private to the extension and is never exposed as tool output; all private dispatch identifiers (worker handle, task id, dispatch id, capability) are additionally redacted from any diagnostic text surfaced to the model. The bridge does not launch workers, create or merge worktrees, rename terminals, proxy other Orca orchestration operations, or provide a generic host adapter.

Worker mode independently enforces the resolved turn limit (`--imp-turn-limit`, default 30, strict whole number ≥ 2) using private in-memory lifecycle state (turn count, last observed assistant output, and exactly-once completion sealing/in-flight guards) reset on every freshly verified dispatch. On the penultimate turn it queues the existing FINAL TURN directive as a steer message; on the final turn it awaits a completion report before aborting the run so the report attempt is never interrupted mid-flight — a failed report simply stays unsealed, leaving the run to settle naturally afterward rather than retrying explicitly. If the run settles without a prior sealed report (e.g. the model never called `agent_done` and the turn limit was never hit), `agent_settled` awaits one last completion attempt; a failure there is swallowed rather than retried, since no parent backend yet observes it. Completion is reported over the internal status subject exactly-once/in-flight-guarded by `reportImpCompletion`, using one of three stable, non-model-controlled subjects — `pi-imps:completed`, `pi-imps:failed`, `pi-imps:truncated` — that a future parent can parse without trusting model-provided text or any embedded identifier.
