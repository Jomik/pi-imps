...
**Imp flags** (global `imps.json` only, `impFlags` key) are names of boolean flags registered by extensions selected for an imp. The default is an empty list. For every imp, pi-imps sets each named flag to `true` before extension session startup: directly in an in-process child session, or as a safely quoted `--<name>` flag on the Orca Pi launch. pi-imps does not interpret flag names or depend on the extension that declares them. Only nonempty valid flag names are accepted; a malformed `impFlags` setting fails settings loading, and a flag missing from the selected extensions or registered as a non-boolean fails that imp's launch before the worker runs. No silent fallback removes a requested policy flag. This is not an arbitrary argv/env transport: no values, shell fragments, project-level overrides, or per-summon flags are supported. Extensions remain responsible for their own flag semantics.
...
- When launched with `--imp-ready-file`, create the marker with mode 0600 after worker-mode hooks are installed during the first `session_start`. The path is internal launch state, never sent to the model; a reused dispatch does not re-create the marker.
...
- Launched with the resolved model, thinking level, and turn limit passed explicitly as CLI arguments (`--model`, `--thinking`, `--imp-turn-limit`), and `--is-imp`, which puts the worker into Orca worker mode instead of ordinary pi-imps: it reports terminal completion and enforces the turn limit locally, but never registers summon/wait/dismiss/list_imps and never discovers agents — no recursive delegation. An internal `--imp-ready-file` flag supplies a path inside that spawn's private prompt directory; worker mode creates that marker only after its first `session_start` has selected worker mode and installed its hooks. Without this flag (including ordinary parent sessions), no marker is created.
- The resolved system prompt is written verbatim to a uniquely named file inside a freshly created private temp directory (`fs.mkdtemp`, mode 0700), with the file itself restricted to mode 0600. Only that file's absolute path — never the prompt content — is passed to `--system-prompt` and appears in the terminal command handed to Orca; Pi natively reads an existing path supplied to `--system-prompt`. After terminal creation, pi-imps waits for the worker-created ready marker before calling `terminal wait --for tui-idle`; the two waits share a 60-second abort-aware startup deadline. A shell-only `tui-idle` result cannot advance to task creation or `worker-start`. The temp file (and its private directory, including the marker) is removed immediately once the ready marker exists and `terminal wait --for tui-idle` reports satisfied (Pi has finished loading it), or immediately on any earlier preparation/startup failure. A failure to remove it is never swallowed: it fails the launch (while existing dispatch/terminal cleanup still runs) rather than silently leaving the prompt on disk.
- `--tools` is passed unchanged only when an explicit tool allowlist was resolved; when there is no explicit allowlist, `--tools` is omitted entirely and every tool among the selected extensions/builtins is available. Configured `impFlags` are passed only after confirming the selected extensions declare them as boolean flags, using the same selection as in-process imps. Worker completion is lifecycle-driven and cannot be configured away.
...
Orca workers currently expose no token or turn telemetry over the orchestration protocol. `list_imps`/`wait` display rows for Orca-dispatched imps therefore omit the turn/token stats suffix entirely instead of showing misleading zero values — local in-process imps are unaffected. LLM-visible JSON output already omits stats, so it is unchanged either way.
