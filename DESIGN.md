...
Grant candidates are project Armory tools that are currently registered in the parent session and are not already available to the agent from its frontmatter, the default allowlist, or global/project grants. The TUI presents them as a searchable multi-selection. Remove candidates are every current project grant, including stale or currently unregistered names, so obsolete grants remain removable; they are presented in a second searchable multi-selection. Remove options show any `agent`, `default`, or `global` sources that will continue providing access after the project grant is removed. Cancellation changes nothing.

Applying a selection persists all selected additions or removals together and affects subsequently summoned imps. Removing a project grant removes only that source; access inherited from another source remains unchanged. Existing settings for other agents and unrecognized tool names are preserved. Missing Armory support, an empty project Armory config, and read or write failures are reported in the TUI; invalid config is never overwritten.
